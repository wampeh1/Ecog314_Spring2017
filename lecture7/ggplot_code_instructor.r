...
data.path <- "/mra/home/m1skm01/Howard_ECOG_314/ggplot2/Data/"
...
plot.data <- treasuries %>% 
    ...
    mutate(DATE = as.Date(DATE, "%Y-%m-%d"),
           GS3 = as.numeric(GS3),
           GS10 = as.numeric(GS10)) %>% 
    select(DATE, GS3, GS10)

first.layer <- ggplot(plot.data, 
                      aes(x = DATE, y = GS3, 
                          color = "3 year")) +
    geom_line()

second.layer <- first.layer +
    geom_line(aes(y = GS10, color= "10 year")) +
    labs(title = "Three and Ten Year Treasury Yields, 1996-2016",
         x = "Date", y = "Interest Rate")
...
scatter <- ggplot(plot.data, aes(x = DATE, 
                                 y = value, 
                                 group = variable))
...
inClass2 <- ggplot(plot.data, aes(x = DATE, 
                                  y = value, 
                                  group = variable,
                                  color = variable)) +
    geom_line(aes(linetype = variable)) +
    geom_point()
# can show scaling and labelling on this chart, set the y-axis to 0 etc
# also, difference between geom_line(aes(linetype = xxx)) vs 
# geom_line(linetype = "yyy")
inClass2
...
inClass6 <- inClass5 + scale_linetype_discrete(name = "Security Tenor", 
                                               labels = c("GS1" = "1 Year",
                                                          "GS3" = "3 Year",
                                                          "GS5" = "5 Year",
                                                          "GS10" = "10 Year",
...
inClass7 <- inClass6 +
    ggtitle(label = "US Treasury Securities",
            subtitle = "For 2016") +
...
inClass8 <- inClass7 + 
            theme(
                  plot.title = element_text(hjust = 0.5, size = 16, color = "green"),
                  axis.title.x = element_text(size = 14, face = "italic", angle = 90, 
                                              color = "red", vjust = 0.25),
                  axis.title.y = element_text(size = 12, face = "bold", color = "blue"),
                  plot.subtitle = element_text(size = 10, face = "bold", color = "orange", hjust = 1)
                  )
